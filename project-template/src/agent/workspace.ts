...
1. **Recall** previous memory with \`recall("project overview")\` — check if context exists from prior sessions.
...
3. **Remember** what you learn:
   ...
   - \`remember("memory", "Dependencies", "...")\` — external libraries
   - \`remember("context", "Project Goals", "...")\` — goals, constraints, stakeholders
4. **Check** for active todos: \`list_memories("todo")\` and plans: \`list_memories("plan")\` — carry over unfinished work.
5. Call \`send_report\` with title "Exploration Complete" summarising what you found, then proceed to the main task.`;
}
...
export async function buildStartupContext(_workspace: string, task: string, isNewProject: boolean): Promise<string[]> {
	...
	// Load relevant memories from vector DB
	try {
