...
/**
 * Centralised browser-rendering for the whole platform. host-api is the only
...
function requireChromium(): string {
	...
	if (!exe) {
		throw new Error(
			"Chromium not found. Install it on the host-api host (e.g. `brew install chromium` " +
...
/** Path to the mermaid-cli binary bundled as a host-api dependency. */
