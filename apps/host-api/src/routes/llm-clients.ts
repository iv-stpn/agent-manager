...
export const llmClientsRouter = new Hono<HonoHostEnv>()
	.get("/", (c) => {
		const clients = c.var.hostDb.listLlmClients().map((client) => ({
			...client,
	...
	.get("/:id/raw", (c) => {
		const client = c.var.hostDb.getLlmClient(c.req.param("id"));
		if (!client) return c.json({ error: "LLM client not found" }, 404);
		return c.json(client);
	})
	.post("/", zValidator("json", CreateLlmClientSchema), async (c) => {
		try {
			const client = c.var.hostDb.createLlmClient(c.req.valid("json"));
	...
	.put("/:id", zValidator("json", UpdateLlmClientSchema), async (c) => {
		try {
			const client = c.var.hostDb.updateLlmClient(c.req.param("id"), c.req.valid("json"));
			return c.json(client);
		} catch (error) {
	...
	.delete("/:id", (c) => {
		try {
			c.var.hostDb.deleteLlmClient(c.req.param("id"));
