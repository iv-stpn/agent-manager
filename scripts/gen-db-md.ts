 ...
 *   - Master  — the orchestrator's own SQLite db (templates, archives, global
 *               stats). Schema lives as raw `CREATE TABLE` SQL in
 *               packages/projects/src/master-database.ts.
...
const MASTER_DB_SRC = join(ROOT, "packages", "projects", "src", "master-database.ts");
...
// ── Master schema (raw CREATE TABLE SQL) ─────────────────────────────────────

function buildMasterTables(): TableDoc[] {
	const { MasterDatabase } = require(join(ROOT, "packages", "projects", "src", "master-database.ts"));
	const tmp = mkdtempSync(join(tmpdir(), "gen-db-md-"));
	const comments = parseMasterComments();

	try {
		const dbInstance = new MasterDatabase(tmp);
...
/** Parse `--` comments from each CREATE TABLE block in the master schema. */
function parseMasterComments(): CommentMap {
	const src = readFileSync(MASTER_DB_SRC, "utf8");
...
/**
 * Which master tables get default rows on first open. Read from the
 * `seedDefaults()` method in master-database.ts so the docs stay in sync with
...
const masterTables = buildMasterTables();
const seededTables = parseSeededTables();

const masterIntro =
	"The orchestrator's own database (`master.db`): templates, archived projects/sessions, and global statistics. " +
	"Defined in `packages/projects/src/master-database.ts`." +
	(seededTables.length
...
const doc = [
	...
	"",
	renderSection("Master", masterIntro, masterTables),
	renderSection(
		"Project",
...
console.log(`Wrote ${OUT_FILE} (${masterTables.length} master + ${projectTables.length} project tables)`);
